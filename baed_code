...
# 2. before applying the code, landsat image should be normalized (Canty and Neilsen et al (2008)) to extract burned area
# 3. there are a total of four sections, see following for details 

############## section 1: download MODIS data and prepare functions
setwd("D:\\dataset\\XinganTM\\Russian\\p122020\\iMAD_p122020\\supp")

library("MODIS")
library(rgdal)
library(raster)

testr1 <- readOGR(dsn = ".", layer = "test_area")

proj.utm = projection(testr1)
proj.geo = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs +towgs84=0,0,0 "

MODISoptions(localArcPath="D:\\dataset\\XinganTM\\Russian\\p122020\\iMAD_p122020",
             outDirPath="D:\\dataset\\XinganTM\\Russian\\p122020\\iMAD_p122020",
...
runGdal(product="MOD13Q1",  #burned area/combined/Tile/500m/monthly
        begin=dates2$beginDOY,
        end = dates2$endDOY,
        tileH = 25,tileV = 3, 
        SDSstring = "111", #only extract the first 2 layers, if want 1 and 3 layer, can be set as 101
        extent = testr1, #crop to extent
        outProj=proj.utm,
        job = "VI")

#read MODIS NDVI data 
ref <- preStack(path=".\\VI", pattern="*NDVI.tif$")
## YearDOYmodis = substr(ref, nchar(ref[1])-27, nchar(ref[1])-21)  ##use this one if evi dataset was used
...
ref.qa <- preStack(path=".\\VI", pattern="*VI_Quality.tif$")
s.qa <- stack(ref.qa)

#find the good measurement, 0 is good measurement, and all the others is not
...
#the following function get multiple date, and also sort it based on length
...
# BEGIN read remote sensing images at ENVI format
nbr.list = list()
ndvi.list = list()
band1.list = list()
band2.list = list()
...
for (i in 1:length(fn)){
  ...
  r = readGDAL(fn[i]) #r is a spatialdataframe
...
poly.list = list() #to store burned patches, if more than 2 times of landsat data is available
for (i in 1:length(nbr.list2.dif)){
#3.1 find burned core pixels and unburned pixels
  
	  ## select burned core pixels
	  burned.core = nbr.list2.dif[[i]] < nbr.threshold & ndvi.list2.dif[[i]] < ndvi.threshold
	  burned.core[cloud.list2[[i]]==1 | cloud.list2[[i+1]]==1] = 0 #remove cloud influence
	  
	#d1.v = freq(di.dif2)
	#if (length(which(d1.v[,1]==1))>0) {
	# if(d1.v[which(d1.v[,1]==1),2] > 1000){
          
      ##select unburned pixels
      di.dif3 = nbr.list2.dif[[i]] > 0 &  ndvi.list2.dif[[i]] > 0
      di.dif3[cloud.list2[[i]]==1 | cloud.list2[[i+1]]==1] = 0 #remove cloud influence
      
#3.2 using brt model to simulate burned probability
#3.2.1 preparing dataset   
      
      di.dif2[di.dif2 == 0] = NA
      di.dif3[di.dif3 == 0] = NA 
      r1.xy = rasterToPoints(di.dif2) #get raster coordinate, from left to right, top to bottom
      r1.xy = data.frame(r1.xy)
      r1.xy.sp <- SpatialPoints(coords = cbind(r1.xy$x,r1.xy$y),proj4string = CRS(proj.utm))
      
      r1.xy2 = rasterToPoints(di.dif3) #get raster coordinate, from left to right, top to bottom
...
#3.2.3 doing a 3 by 3 windows smoothing
...
for (i in 1:length(poly.list)){
    writeOGR(poly.list[[i]], ".", "test_area_fire", driver="ESRI Shapefile", overwrite_layer=TRUE)
...
writeOGR(firep, ".", "test_area_fire_dated", driver="ESRI Shapefile", overwrite_layer=TRUE)
